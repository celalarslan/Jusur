...
import { Mic, MicOff, PhoneOff, Globe, Sparkles, Volume2, VolumeX, ShieldAlert, Loader2 } from "lucide-react";
...
export function ActiveCallScreen({ call, onHangUp, currentUserEmail }: ActiveCallScreenProps) {
  ...
  const [myLanguage, setMyLanguage] = useState("auto"); // "auto" for Auto-detect
  const [partnerLanguage, setPartnerLanguage] = useState("es");
  ...
  // Establish WebRTC Voice Connection via Firestore Signaling
  useEffect(() => {
    let active = true;
    const stunServers = { iceServers: [{ urls: "urls:stun.l.google.com:19302" }] };
    ...
    const setupConnection = async () => {
      try {
        ...
        // When remote track is received, route it
        pc.ontrack = (event) => {
        ...
        if (isCaller) {
          ...
          pc.onicecandidate = async (e) => {
            if (e.candidate === null) {
              // ICE Gathering complete (Vanilla ICE)
          ...
          // Standard Timeout fallback: if no answer exists, offer simulation fallback / loops
          setTimeout(() => {
            if (active && pc.signalingState !== "stable" && pc.iceConnectionState !== "connected") {
              console.log("No WebRTC answer in timeframe. Activating loopback simulation test mode.");
              setConnectionStatus("loopback_mode");
            }
          }, 8000);

        } else {
          // RECEIVER FLOW: Read Caller's Offer from document manually or from call info
          // We wait for the offer to be present in the document.
          // Note: To simplify structure, we set receiver to answer the offer.
          console.log("Receiver reading SDP offer...");
          // In React, parent updates incoming call block. Let's process signature directly:
          pc.onicecandidate = async (e) => {
            if (e.candidate === null) {
  ...
  // Listen to Firestore signaling changes for WebRTC exchange
  useEffect(() => {
    // If we have direct updates on call document, subscribe
    let lastCallerSignal = "";
    let lastReceiverSignal = "";

    const pc = peerConnectionRef.current;
    if (!pc) return;

    // We can use native polling or simple intervals to monitor signaling
    const signalInterval = setInterval(async () => {
      try {
        // Query current state
        const res = await fetch(`/api/meet/create-space`); // Check if we need to retrieve, or we can get from Firestore through active listeners.
        // Wait, since we are already listening to calls in App.tsx, we can check for state updates.
        // Actually, let's keep it highly modular. Let's subscribe to the single document updates if needed.
        // Let's implement signaling directly in ActiveCallScreen if App.tsx passes updates,
        // or we check Firestore updates. Wait, we can implement it as a simple polling read of `/calls/{id}` or use Firestore!
        // We have `listenSingleCall` import in App.tsx. Can we listen directly here using standard firestore?
        // Yes, but a polled fetch or simple check of callerSignal/receiverSignal is extremely reliable!
        // Wait, even better: we can read call document directly.
      } catch (e) {
        // Ignore
      }
    }, 1500);
  ...
  useEffect(() => {
    ...
    const applySignaling = async () => {
      try {
        ...
        // If Receiver: Wait for callerSignal
        if (!isCaller && call.callerSignal && pc.signalingState === "stable") {
          // Process updates
        }
      } catch (e) {
  ...
  const startInterpreter = async () => {
    try {
      ...
      // Joint processing node
      const processor = audioCtx.createScriptProcessor(4096, 1, 1);
      scriptProcessorRef.current = processor;

      // Connect sources to processing node (Web Audio automatically mixes them)
      if (localSourceRef.current) {
        localSourceRef.current.connect(processor);
      }
      
      // If loopback simulation, loopback localSource as both local and remote
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (msg.audio) {
            playDecodedAudioChunk(msg.audio);
          }
  ...
  const playDecodedAudioChunk = (base64Data: string) => {
  ...
  return (
    <div className="absolute inset-0 z-20 flex flex-col bg-neutral-950 font-sans text-neutral-100 rounded-2xl border border-white/5 overflow-hidden shadow-[0_0_50px_rgba(37,99,235,0.12)]">
      {/* Hidden audio tag for Standard WebRTC incoming caller voice */}
      <audio ref={remoteAudioRef} className="hidden" autoPlay playsInline />

      {/* Header element */}
      <div className="flex justify-between items-center px-5 py-4 border-b border-white/5 bg-neutral-900/85 backdrop-blur-md shadow-md shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className="text-zinc-500 text-[9px] font-mono tracking-widest uppercase block mb-0.5">Translation Stream • Synchronized</span>
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-tr from-rose-600 to-red-700 hover:from-rose-500 hover:to-red-600 text-xs font-bold text-white transition-all active:scale-95 cursor-pointer shadow-lg shadow-rose-950/40"
        >
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-h-0 divide-y divide-white/5">
        ...
        {/* Upper portion: Peer state displays & Live Interpreter toggle */}
        <div className="p-5 bg-neutral-900/30 flex flex-col gap-4 shrink-0">
          
          <div className="flex items-center justify-between bg-neutral-950/40 p-3.5 rounded-xl border border-white/5">
            <div className="leading-tight">
              <p className="text-[9px] text-zinc-500 font-mono tracking-wider">CHANNEL PROTOCOL / HEALTH</p>
            ...
            <div className="text-right">
              <span className="text-[9px] text-zinc-500 font-mono tracking-wider block mb-1">INTERPRETER DECODE</span>
              <span className="text-xs font-bold text-amber-500 bg-amber-500/10 px-2.5 py-0.5 rounded-full border border-amber-500/15">
                {partnerMuted ? "Active (Vocalizing Live)" : "Auto Monitor"}
              </span>
            </div>
          </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-xs font-black tracking-tight text-white flex items-center gap-2 font-display">
                    ...
                    <span className="text-[8px] font-mono tracking-widest uppercase bg-gradient-to-r from-blue-950 to-indigo-950 text-blue-400 px-2 py-0.5 rounded-full font-black border border-blue-900/30">Gemini 3.5 Live</span>
                  </h3>
